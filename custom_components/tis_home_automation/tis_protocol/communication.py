...
class TISUDPTransport(TISTransport):
    ...
    async def broadcast_discovery(self, source_ip: str) -> bool:
        """Send UDP discovery broadcast - SIMPLE APPROACH like UDP sniffer"""
        try:
            # Build simple discovery packet (multiple OpCodes like before)
            discovery_opcodes = [0x000E, 0xF003, 0xDA44, 0x0002]
            
            for op_code in discovery_opcodes:
                discovery_packet = build_packet(
                    operation_code=[(op_code >> 8) & 0xFF, op_code & 0xFF],
                    ip_address=source_ip,
                    source_device_id=[0x01, 0xFE],
                    device_type=[0xFF, 0xFE]  # Like UDP sniffer saw
                )
                ...
                if self.socket:
                    ...
                    _LOGGER.info(f"UDP discovery sent OpCode 0x{op_code:04X}")
    ...
    async def _receive_loop(self):
        """Background task to continuously receive UDP packets - SIMPLE like UDP sniffer"""
        _LOGGER.info("Starting UDP receive loop (sniffer style)")
        try:
            while self._running and self.socket:
                try:
                    # Simple receive like UDP sniffer
                    self.socket.settimeout(1.0)
                    data, addr = await asyncio.get_event_loop().run_in_executor(
                        None, self.socket.recvfrom, 1024
                    )
                    
                    _LOGGER.info(f"UDP received {len(data)} bytes from {addr[0]}:{addr[1]}")
                    _LOGGER.debug(f"Raw packet: {hexstr(data)}")
                    
                    # Use UDP sniffer's parse logic
                    parsed = self._parse_smartcloud_simple(data)
                    if parsed:
                        op_code = parsed['op_code']
                        _LOGGER.info(f"SMARTCLOUD: {parsed['source_device']} -> {parsed['target_device']}")
                        _LOGGER.info(f"OpCode: 0x{op_code:04X}, DevType: 0x{parsed['device_type']:04X}")
                        
                        # Call discovery callback for ANY OpCode (like UDP sniffer captures all)
                        for callback in self._response_callbacks.values():
                            try:
                                if asyncio.iscoroutinefunction(callback):
                                    await callback(parsed, addr)
                                else:
                                    callback(parsed, addr)
                                _LOGGER.info(f"Called discovery callback for OpCode 0x{op_code:04X}")
                            except Exception as e:
                                _LOGGER.error(f"Callback error: {e}")
                    else:
                        _LOGGER.debug(f"Non-SMARTCLOUD packet: {len(data)} bytes")
                    
                except socket.timeout:
                    continue
                except Exception as e:
                    if self._running:
                        _LOGGER.error(f"UDP receive error: {e}")
                    continue
                    
        except asyncio.CancelledError:
            _LOGGER.info("UDP receive loop cancelled")
        except Exception as e:
            _LOGGER.error(f"UDP receive loop failed: {e}")
    
    def _parse_smartcloud_simple(self, packet_data):
        """Simple SMARTCLOUD parser like UDP sniffer"""
        try:
            if len(packet_data) < 29:
                return None
            
            # Check for SMARTCLOUD header
            header = packet_data[4:14]
            if header != b'SMARTCLOUD':
                return None
            ...
            # Target device (2 bytes) - FIXED
            target_device = [packet_data[23], packet_data[24]]
            
            # Additional data
            additional_data_length = length - 11
            additional_data = b''
            if additional_data_length > 0 and len(packet_data) > 25:
                end_pos = min(25 + additional_data_length, len(packet_data))
                additional_data = packet_data[25:end_pos]
            
            return {
                'valid': True,
                'ip': ip,
                'source_device': source_device,
                'target_device': target_device,  # ADDED
...
class TISCommunicationManager:
    ...
    async def discover_devices(
        ...
        for device_key, device in self.discovered_devices.items():
            _LOGGER.info(f"   📱 {device_key}: {device.model_name} (Type: 0x{device.device_type:04X})")
    ...
    async def _handle_discovery_response(self, parsed_packet: Dict, source_addr: Any):
        """Handle discovery response packets - SIMPLE UDP sniffer approach"""
        try:
            _LOGGER.info(f"🔍 DISCOVERY RESPONSE: {parsed_packet}")
            
            # Extract device information (UDP sniffer style)
            ...
            ip_address = parsed_packet.get('ip', "unknown")
            
            _LOGGER.info(f"📱 Device: ID={device_id}, Type=0x{device_type:04X}, OpCode=0x{op_code:04X}")
            _LOGGER.info(f"📍 IP={ip_address}, Data={len(additional_data)} bytes")
            
            # Create device key
            if isinstance(device_id, list) and len(device_id) >= 2:
                device_key = f"{device_id[0]:02X}{device_id[1]:02X}"
            else:
                device_key = f"DEV_{device_type:04X}"
            ...
            # Create device name
            if not device_name:
                device_name = f"TIS Device {device_key}"
            
            # Create TIS device (correct parameters)
            device = TISDevice(
                device_id=device_key,  # Use string device key
                device_type=device_type,
                model_name=device_name if device_name else f"TIS Device {device_key}",
                ip_address=ip_address,
                transport="udp",
                comment=device_name  # Store decoded name as comment
            )
            
            # Store device
            self.discovered_devices[device_key] = device
            
            _LOGGER.info(f"[SUCCESS] DEVICE FOUND: '{device.model_name}' ({device_key}) @ {ip_address}")
            ...
            _LOGGER.error(f"Discovery response error: {e}")
            import traceback
